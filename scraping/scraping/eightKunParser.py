...
class EightKunPageScraper:
    ...
    def parse_content(self, parser="html.parser"):
        return BeautifulSoup(self._raw_content, parser)
    ...
    def get_posts(self):
        posts = self.parse_content().find_all('div', {
            'class': 'body'})
        pl = [tag.find_all('p', {'class': 'body-line'}) for tag in posts]
